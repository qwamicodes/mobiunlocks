...
export class Pay {
    ...
    calcPrice() {
        switch (this.type) {
            case "carrier":
                if (
                    this.network === "at&t" ||
                    this.network === "verizon" ||
                    this.network === "orange" ||
                    this.network === "ee" ||
                    this.network === "tmobile-uk"
                ) {
    ...
    insertPay() {
        ...
        document
            .querySelector(elements.paymentItems)
            .insertAdjacentHTML("beforeend", cautionHtml);
...
export const filterList = tab => {
    ...
    if (tab.getAttribute("data-value") === "all") {
        ...
        const lists = items.filter(
            item =>
                item.lastElementChild.getAttribute("data-type") !==
...
export const setAdminDetailsOnDashboard = () => {
    ...
    api.getAdminDetails(authUserEmail)
        .then(adminDetails => {
            ...
            ).innerText = adminDetails.username
                ? adminDetails.username
                : "No Username";
            document.querySelector(
                elements.adminAvatarImage
            ).src = adminDetails.user_avatar
                ? adminDetails.user_avatar
                : document.querySelector(elements.adminAvatarImage).src;
        })
        .catch(error => {
            if (error === "unauthorized")
                api.refreshToken().then(setAdminDetailsOnDashboard());
            else {
...
export const listTasksOnDashboard = async () => {
    ...
    let allTasksListElement = document.querySelector(
        elements.allTasksListElement
    );
    ...
    await api
        ...
        .then(tasks => {
            ...
            <ul class="dashboard__tasks--item">
                ...
                <li data-task_property="phone_model">${
                    task.phone_model ? task.phone_model : "---"
                }</li>
                <li data-task_property="imei">${task.imei}</li>
                <li data-task_property="carrier">${
                    task.phone_carrier_network
                        ? task.phone_carrier_network
                        : task.details
                        ? task.details
                        : "---"
                }</li>
                <li data-task_property="status" data-type="${
            ...
            `;
                ...
                Array.prototype.slice
                    .call(singleTask.getElementsByTagName("li"))
                    .forEach(item => {
                        item.addEventListener("click", e => {
                            ...
                            document
                                .querySelector(elements.taskDetailModalForm)
                                .addEventListener("submit", updateTaskDetails);
                        });
                    });
            });

            // TODO Task count
            document.querySelector(elements.allTasksCount).innerHTML =
                tasks.length; // all tasks
            document.querySelector(
                elements.pendingTasksCount
            ).innerHTML = countPendingTasks(tasks); // pending tasks
            document.querySelector(
        ...
        .catch(error => {
            console.log(error);
            if (error === "unauthorized")
                api.refreshToken().then(listTasksOnDashboard());
            else {
                UICtrl.popupAlert(
...
export const updateTaskDetails = async e => {
    ...
    const trackingID = e.target
        .querySelector('span[data-task_property="tracking_id"]')
        .innerText.slice(1);
    const imei = e.target.querySelector('span[data-task_property="imei"]')
        .innerText;
    const completed =
        e.target.elements.status.value === "completed" ? true : false;
    const taskType = e.target
        .querySelector('span[data-task_property="task_type"]')
    ...
    const correspondingTabElement = document.querySelector(
        `${elements.taskTab}[data-value="${taskStatus}"]`
    );
        ...
        case "icloud unlocking":
            api.updateICloudUnlockTask(trackingID, formData)
                .then(async () => {
                    ...
                    const taskHTMLElement = document.getElementById(
                        `task_${trackingID}`
                    );
                ...
                .catch(error => {
                    // console.log(error);
                    if (error === "unauthorized")
                        api.refreshToken().then(updateTaskDetails(e));
                    else {
                        UICtrl.popupAlert(error, "error");
                    }
                });
